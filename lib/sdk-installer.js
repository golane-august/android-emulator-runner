...
const PLATFORM_TOOLS_VERSION = '31';
...
function installAndroidSdk(apiLevel, target, arch, channelId, emulatorBuild, ndkVersion, cmakeVersion) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        core.addPath(`${process.env.ANDROID_SDK_ROOT}/emulator:${cmdlineToolsPath}/latest:${cmdlineToolsPath}/latest/bin:${process.env.ANDROID_SDK_ROOT}/platform-tools`);
        // set standard AVD path
        core.exportVariable('ANDROID_AVD_HOME', `${process.env.HOME}/.android/avd`);
        // accept all Android SDK licenses
        yield exec.exec(`sh -c \\"yes | sdkmanager --licenses > /dev/null"`);
        console.log('Installing latest build tools, platform tools, and platform.');
        yield exec.exec(`sh -c \\"sdkmanager --install 'build-tools;${BUILD_TOOLS_VERSION}' platform-tools 'platforms;android-${PLATFORM_TOOLS_VERSION}' > /dev/null"`);
        console.log('Update SDK Manager packages.');
        yield exec.exec(`sh -c \\"sdkmanager --update --channel=${channelId} > /dev/null"`);
        if (emulatorBuild) {
